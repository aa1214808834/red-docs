...
Red/View (简称 View)组件是Red编程语言的图形系统。它的设计目标是：

* 面向数据，最小化的API
* 以虚拟对象树为编程接口 原文:Virtual tree of objects as programming interface
* 在虚拟树和显示系统之间实时或延迟同步 原文:Realtime or deferred synchronization between the virtual tree and the display system
* 支持更细化的双向绑定 原文:Make two-way binding trivial to support
* 跨平台后端 Ability to have different backends, across different platforms
* 支持操作系统、第三方和自定义的窗口部件
* 低性能开销

虚拟树使用 face 对象构造. 每一个 face 对象以双向绑定方式映射到显示屏上的图形组件。
